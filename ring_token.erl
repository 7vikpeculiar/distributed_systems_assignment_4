...
zero_loop(Val) ->
    ...
    receive
        {send_token, {OutFileName2, SrcPid2, Us2, Token_val2}} ->
            ...
            io:fwrite(IoDevice,"Process ~p received token ~p from process ~p.",[Us2, Token_val2, SrcPid2]),
            file:close(IoDevice);
        _ -> % All other messages
            loop(Val)
    end.
...
mini_loop(1,M) ->
    register(list_to_atom(integer_to_list(0)),spawn(ring_token, zero_loop, [M]));

mini_loop(N,M) ->
    register(list_to_atom(integer_to_list(N-1)),spawn(ring_token, loop, [M])),
    mini_loop(N-1,M).
